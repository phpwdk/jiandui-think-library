...
if (!function_exists('this2arr')) {
     ...
     * @return array
     */
    function this2arr($data): array
    {
        foreach ($data as $name => $value) $vars[$name] = $value;
        return $vars ?? [];
    }
}
...
if (!function_exists('json_decode_array')) {
     ...
     * @return array
     */
    function json_decode_array(string $data): array
    {
...
if (!function_exists('post_field_filter')) {
    /**
     * POST字段过滤
     *
     * @param string $field
     ...
     * @return void
     */
    function post_field_filter(string $field): void
    {
        $request = app()->request->post();
        foreach (str2arr($field) as $field) if (isset($request[$field])) unset($request[$field]);
        app()->request->withPost($request);
    }
}
