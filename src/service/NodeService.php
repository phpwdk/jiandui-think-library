...
class NodeService extends Service
{
    ...
    private function parseComment(string $comment, string $default = ''): array
    {
        ...
        if (in_array(substr($title, 0, 5), ['@auth', '@menu', '@logi', '@user', '@merc'])) $title = $default;
        return [
            'title'      => $title ?: $default,
            ...
            'isuser'     => intval(preg_match('/@user\s*true/i', $text)),
            'ismerchant' => intval(preg_match('/@merchant\s*true/i', $text)),
        ];
    }
