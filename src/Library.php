            ...
            $this->app->middleware->add(function (Request $request, Closure $next) {
                ...
                if (($origin = $request->header('origin', '*')) !== '*') {
                    ...
                    if ($this->app->config->get('app.cors_auto', 1) || in_array(parse_url(strtolower($origin), PHP_URL_HOST), $hosts)) {
                        $headers                                    = $this->app->config->get('app.cors_headers', 'Api-Name,Api-Type,Api-Token,User-Form-Token,User-Token,Access-Token,Token');
                ...
                if ($request->isOptions()) {
                    ...
                    AdminService::instance()->isLogin() ||
                    AdminService::instance()->isAppLogin() ||
                    AdminService::instance()->isAppLogin('USER')
                ) {
                    return json(['code' => 403, 'info' => lang('think_library_not_auth')])->header($header);
                } else {
                    return json(['code' => 401, 'info' => lang('think_library_not_login'), 'url' => sysuri('admin/login/index')])->header($header);
                }
            }, 'route');
        }
    }
