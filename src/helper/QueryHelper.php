...
class QueryHelper extends Helper
{
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function ajaxTable(?callable $befor = null, ?callable $after = null, string $template = '')
    {
        if (in_array($this->output, ['get.json', 'get.ajax.table'])) {
            if (is_callable($after)) {
                call_user_func($after, $this, $this->query);
            }
            $this->page->ajaxTable($this->query, $template);
        } else {
            if (is_callable($befor)) {
                call_user_func($befor, $this, $this->query);
            }
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function ajax(?callable $after = null, ?array $input = null)
    {
        if (is_callable($after)) {
            call_user_func($after, $this, $this->query);
        }
        $this->page->ajax($this->query, $input);
    }
