...
class PageHelper extends Helper
{
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function ajaxTable($dbQuery, string $template = ''): array
    {
        if ($this->output === 'get.json') {
            return PageHelper::instance()->init($dbQuery);
        }
        if ($this->output === 'get.ajax.table') {
            $get   = $this->app->request->get();
            $query = $this->autoSortQuery($dbQuery);
            // 根据参数排序
            if (isset($get['_field_']) && isset($get['_order_'])) {
            ...
            if (empty($get['page']) || empty($get['limit'])) {
                $data   = $query->select()->toArray();
                $result = ['info' => '', 'code' => 1, 'count' => count($data), 'data' => $data];
            } else {
                $cfg    = ['list_rows' => $get['limit'], 'query' => $get];
                $data   = $query->paginate($cfg, $this->getCount($query))->toArray();
                $result = ['info' => '', 'code' => 1, 'count' => $data['total'], 'data' => $data['data']];
            }
            $this->xssFilter($result['data']);
            if (false !== $this->class->callback('_page_filter', $result['data'], $result)) {
                throw new HttpResponseException(json($result));
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function ajax($dbQuery, ?array $input = null): array
    {
        $query = $this->autoSortQuery($dbQuery);
        // 根据参数排序
        if (isset($input['_field_']) && isset($input['_order_'])) {
            $query->order("{$input['_field_']} {$input['_order_']}");
        }
        // 数据分页处理
        if (empty($input['page']) || empty($input['limit'])) {
            $data   = $query->select()->toArray();
            $result = ['info' => '', 'code' => 1, 'count' => count($data), 'data' => $data ?: null];
        } else {
            $cfg    = ['list_rows' => $input['limit'], 'query' => $input];
            $data   = $query->paginate($cfg, $this->getCount($query))->toArray();
            $result = ['info' => '', 'code' => 1, 'count' => $data['total'], 'data' => $data['data'] ?: null];
        }
        $this->xssFilter($result['data']);
        if (false !== $this->class->callback('_page_filter', $result['data'], $result)) {
            throw new HttpResponseException(json($result));
    ...
    public function autoSortQuery($dbQuery): Query
    {
        ...
        if ($this->app->request->isPost() && $this->app->request->post('action') === 'sort') {
            if (!AdminService::instance()->isManageLogin()) {
